...
export const deploymentWorker = new Worker<DeploymentJobPayload>(
  ...
  async (job) => {
    logger.info('deployment job received', { jobId: job.id, deploymentId: job.data.deploymentId });
    await stateService.markBuilding(job.data.deploymentId);

    await stateService.appendLog(job.data.deploymentId, 'Deployment started');
    ...
    try {
      ...
      logger.error('deployment failed', { deploymentId: job.data.deploymentId, message });
      throw error;
    }
  },
  {
