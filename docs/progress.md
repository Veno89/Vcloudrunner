...
- **Phase 2: Production readiness foundation** — ~50% complete
  - done: improved failure taxonomy coverage, regression tests around constraint/error mapping paths, stronger cancellation/auth/config resilience under partial failures, direct route-level authorization coverage across the main API surfaces, alert-monitor/operational-threshold unit coverage, idempotent alert-monitor startup behavior, and clearer operator-facing startup/config guidance
  - left (~50%): deeper observability dimensions, migration safety gates, backup/restore automation checks, worker/service decomposition, and broader operational validation
