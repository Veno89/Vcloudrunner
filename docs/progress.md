...
- **Phase 2: Production readiness foundation** — ~55% complete
  - done: improved failure taxonomy coverage, regression tests around constraint/error mapping paths, stronger cancellation/auth/config resilience under partial failures, direct route-level authorization coverage across the main API surfaces including SSE log streaming, alert-monitor/operational-threshold unit coverage, idempotent alert-monitor startup behavior, direct API health/shutdown regression coverage, fuller top-level project route auth-matrix coverage, direct server-metrics contract coverage with explicit unavailable mapping, and clearer operator-facing startup/config guidance
  - left (~45%): deeper observability dimensions, migration safety gates, backup/restore automation checks, worker/service decomposition, and broader operational validation
