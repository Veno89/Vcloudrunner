...
- extract worker container/network lifecycle behind a container-runtime-manager seam too, so deployment orchestration no longer binds directly to `dockerode` for network lookup/create, stale-container cleanup, or container start/teardown flows and future runtime adapters have a cleaner hook point
- extract worker workspace preparation/cleanup behind a deployment-workspace-manager seam too, so runtime orchestration no longer owns direct `fs/promises` workdir setup/teardown flows and future runtime adapters have a cleaner hook point
- extract build-file repository inspection behind a repository-file-inspector seam too, so Dockerfile detection no longer shells out to git directly and future repository/build detectors have a cleaner hook point
...
- extract deployment-state repository construction behind a dedicated factory seam too, so state-service composition no longer names `DeploymentStateRepository` directly and future state backends have a cleaner injection point
- extract deployment-state database-queryable construction behind a dedicated factory seam too, so repository composition no longer names `pg` pool construction inline and future state backends have a cleaner injection point
...
- extract duplicated worker runtime-family selection behind a shared resolver seam too, so runtime executor, runtime inspector, and container-runtime-manager factories now share one future-friendly executor-type decision point instead of repeating the same branching inline
- extract the default build-detector list behind a dedicated factory seam too, so configured build-system resolution no longer names `DockerfileBuildDetector` directly and future detector stacks have a cleaner extension point
- extract shared process-launch behavior behind an exec-file runner seam too, so Git repository inspection and shell deployment command execution no longer each name raw `execFile` directly and future process-backed adapters have a cleaner hook point
